...
| `POST` | `/api/v1/dashboards/{id}/selectors/regenerate-mapping` | AI-регенерация одного маппинга селектор→чарт (для клика по соединению на доске селекторов). Body: RegenerateMappingRequest (dc_id, selector_name, selector_label, selector_type, operator, user_request?). Возвращает RegenerateMappingResponse с полями target_column/target_table/operator_override/post_filter_*. AI получает контекст всех чартов дашборда — пользователь может ссылаться на сосед-чарт по названию в user_request |
...
class AIService:
    ...
    async def regenerate_mapping(schema_context: str, charts_context: str, target_dc_id: int, selector_name: str, selector_label: str, selector_type: str, selector_operator: str, user_request: str | None = None) -> dict  # AI-регенерация одного маппинга селектор→чарт. Возвращает {target_column, target_table?, operator_override?, post_filter_resolve_table?, post_filter_resolve_column?, post_filter_resolve_id_column?}. user_request может ссылаться на соседний чарт по названию — backend подставляет контекст всех чартов дашборда
