...
- **selectors/SelectorEditorSection.tsx** — секция управления селекторами в редакторе дашборда: список с сортировкой, кнопки edit/delete, открытие SelectorBoardDialog. AI-панель генерации (кнопка "AI: сгенерировать"): textarea с пожеланием пользователя + чек-лист чартов с кнопками "Выбрать все"/"Снять все" (по умолчанию выбраны все чарты, headings исключены) — выбранные dashboard_chart_id передаются в `dashboardsApi.generateSelectors(dashboardId, userRequest?, chartIds?)`; если выбраны все, chartIds не передаётся (backend использует все чарты)
