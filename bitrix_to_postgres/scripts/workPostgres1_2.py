...
async def create_table_from_fields(table_name, fields_list):
    ...
    for field in fields_list:
        ...
        if field_type == 'string':
            attrs[field_name] = Column(String, comment=description)
        if field_type == 'datetime':
            attrs[field_name] = Column(DateTime, comment=description)
        if field_type == 'integer':
            attrs[field_name] = Column(BigInteger, comment=description)
        if field_type == 'float':
            attrs[field_name] = Column(Float, comment=description)
        if field_type == 'boolean':
            attrs[field_name] = Column(BOOLEAN, comment=description)
        if field_type == 'url':
            attrs[field_name] = Column(String, comment=description)
        if field_type == 'json':
            attrs[field_name] = Column(JSON, comment=description)
        if field_type == 'array':
            attrs[field_name] = Column(ARRAY(String), comment=description)
        else:
            attrs[field_name] = Column(String, comment=description)
...
async def insert_record(table_name: str, data: dict):
    """
    Вставляет запись в указанную таблицу с преобразованием типов данных только для тех полей, которые есть в таблице елси нет то добавляется в таблицу иначе обновляется
    ...
    # if 'bitrix_id' in converted_data:
    #     query = text(f'SELECT record_id FROM "{table_name}" WHERE bitrix_id = :bitrix_id')
    #     async with engine.begin() as conn:
    #         result = await conn.execute(query, {'bitrix_id': converted_data['bitrix_id']})
    ...
    #             """)
    #             await conn.execute(update_query, {**converted_data, 'record_id': existing_id})
    #             await conn.commit()
    #             return existing_id
