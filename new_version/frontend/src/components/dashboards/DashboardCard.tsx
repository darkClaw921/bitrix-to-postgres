      ...
      <div className="flex justify-between items-start">
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center space-x-3 mt-2 text-xs text-gray-400">
            <span>{dashboard.chart_count} {dashboard.chart_count !== 1 ? t('dashboardCard.charts') : t('dashboardCard.chart')}</span>
            <span>{new Date(dashboard.created_at).toLocaleDateString()}</span>
            {!dashboard.is_active && (
              <span className="text-red-400">{t('dashboardCard.inactive')}</span>
            )}
          </div>
        </div>
        ...
        <div className="flex space-x-1 ml-3">
          <button
            ...
            title={t('dashboardCard.openPublic')}
          >
          ...
          <button
            ...
            title={t('dashboardCard.editDashboard')}
          >
            {t('common.edit')}
          </button>
          <button
            ...
            title={t('dashboardCard.copyLink')}
          >
            {copied ? t('common.copied') : t('dashboardCard.link')}
          </button>
          <button
            onClick={() => {
              if (confirm(t('dashboardCard.confirmDelete'))) deleteDashboard.mutate(dashboard.id)
            }}
